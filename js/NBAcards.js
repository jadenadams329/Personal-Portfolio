...
const theData = getAPIData('https://www.balldontlie.io/api/v1/teams').then(data => {
    allTeams = data.data
    westernConference = filterTeams(allTeams, "West")
    easternConference = filterTeams(allTeams, "East")
    populateDOM(allTeams)
})


//Filter
function filterTeams(allTeams, conferenceAffiliation) {
    return allTeams.filter(team => team.conference === conferenceAffiliation)
}
